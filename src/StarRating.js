...
export default function StarRating({maxRating = 5, color = '#fcc419', size='48', className='', messages = [], defaultRating = 0, onSetRating}){
    ...
    return (
        <div style={containerStyle} className={className}>
            <div style={starContainerStyle}>{Array.from({length: maxRating},(_, i)=><Star key={i} onRate ={()=>{setRating(i+1); onSetRating(i+1)}} full={tempRating? tempRating >=
            i+1 : rating >= i+1}
                onHoverIn={()=>setTempRating(i+1)} onHoverOut={()=>setTempRating(0)} color={color} size={size}/>)}</div>
